...
rsync -avtbr -e 'ssh' --rsync-path='sudo rsync' --files-from='/$PATH_TXT/archivos_a_respaldar.txt' --exclude-from='/$PATH_SCRIPTS/archivos_a_omitir.txt' --delete-excluded --filter='protect <carpeta_respaldo>*' <alias_servidor>:/  /$PATH_BACKUP/<carpeta_respaldo>
